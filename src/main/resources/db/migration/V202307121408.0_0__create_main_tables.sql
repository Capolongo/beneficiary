...
CREATE TABLE ORDERS (
    ...
    PARTNER_ORDER_ID VARCHAR2(500) NULL, -- verificar com relação ao token da cvc - Defini como text, pois pode haver uma quantidade não previsível para outros parceiros
...
CREATE INDEX ORDER_PARTNER_ORDER_ID_INDEX on ORDERS(PARTNER_ORDER_ID) online;
...
CREATE TABLE PAX (
    ...
    BIRTH_DATE VARCHAR2(255), -- esse cara não precisa de timezone pois não vai armazenar hora, apenas DD/MM/YYYY
    DOCUMENT VARCHAR2(255) NULL,
    DOCUMENT_TYPE VARCHAR2(10) NULL,
...
CREATE INDEX SEGMENT_ORDER_ITEM_ID_INDEX on SEGMENT(ORDER_ITEM_ID) online;
